"""
experiments/exp_sas.py — SAS ablation suite for p and q combinations.

Context / Protocol
------------------
  • Fixed dimensions: n=100, sn=0.95.
  • Sweeping all valid combinations of p in {0, 1} and q in {0, 1, 2}.
  • Excludes p=0, q=0 (which acts as a dead reservoir, disconnected from input).
...
def _sas(n=100, sn=0.95, p=1, q=1):
...
def build_pq_grid() -> dict[str, callable]:
    """
    Builds a dictionary of model factories for all combinations of
    p in {0, 1} and q in {0, 1, 2}, excluding the null case (p=0, q=0).
    """
    ...
    for p in [0, 1]:
        for q in [0, 1, 2]:
            ...
            name = f"p{p}_q{q}_n200_sn95"
            # Important: Capture loop variables using default arguments 
            # to avoid Python's late-binding closures inside loops.
            grid[name] = lambda p_val=p, q_val=q: _sas(n=200, sn=0.95, p=p_val, q=q_val)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Focused SAS ablation: fully sweep combinations of p and q.",
    ...
    grid_dict = build_pq_grid()
    model_order = list(grid_dict.keys())  # Keep dictionary insertion order (HAR first)

    print(f"\n{'═'*70}")
    print(f"  P and Q exhaustive sweep (n=200, sn=0.95)")
    print(f"{'═'*70}\n")
