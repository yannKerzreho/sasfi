"""
expv2/exp_sas_clip.py — SAS ablation: clipping and input-scaling.

Grid: q ∈ {1,2}  ×  {noclip/1.0, clip/2.0, clip/4.0, clip/6.0}
      + HAR as benchmark.
...
def main():
    ...
    out_dir = Path(args.out_dir) if args.out_dir else ROOT / "experiments/results_sas_clip"
    out_dir.mkdir(parents=True, exist_ok=True)

    grid = build_clip_grid()
    run_grid(
        syms, grid, list(grid.keys()),
        "Clipping & Scaling Ablation",
